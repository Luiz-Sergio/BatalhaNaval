...
public class GameController {
	...
	/**
	 * matrix of integers to signal player unoccupied positions, position of the
	 * ships and position of the shots
	 */
	private int[][] occupiedPositions;
	/**
	 * matrix of integers to signal enemy unoccupied positions, position of the
	 * ships and position of the shots
	 */
	...
	// private Image hitShipImage = new
	// Image(GameController.class.getResourceAsStream("hitShip.jpg"));
	// private ImagePattern hitShipPattern = new ImagePattern(hitShipImage);

	private Image hitOceanImage = new Image(GameController.class.getResourceAsStream("hitOcean.jpg"));
	private ImagePattern hitOceanPattern = new ImagePattern(hitOceanImage);

	/**
	 * initialize game scenario, such as creating and painting board and
	 * instantiating ships
	 */
	@FXML
	public void initialize() {
		...
		Image backgroundImage = new Image(GameController.class.getResourceAsStream("tileset.png"));
	...
	/**
	 * alternates the visibility of the ships based on the previous value of
	 * enemyVisible
	 * 
	...
	public void pressed(MouseEvent event, Ship ship) {
		...
		// used to determine which matrix of occupied position to use based on the
		// parent's name
		...
		if (!mousePressed) {
			if (event.isSecondaryButtonDown()) {
				...
				if (ship.getVertical() && ((ship.getX() / dimension) + ship.getTamanho() <= 10)) {
					if (ship.verifyOccupiedPositions((int) ship.getX() / dimension, (int) ship.getY() / dimension,
							ocPositions, true)) {
						occupyPositions((int) ship.getX() / dimension, (int) ship.getY() / dimension,
								ship.getVertical(), ship.getTamanho(), ocPositions, 0);
						ship.rotate();
						occupyPositions((int) ship.getX() / dimension, (int) ship.getY() / dimension,
								ship.getVertical(), ship.getTamanho(), ocPositions, ship.getTamanho());
					} else {
	...
	/**
	 * moves and draws the ship in the current position if its within borders
	 * otherwise print in the last valid position
	 * 
	 * @param event the mouse event
	 * @param ship  the ship being pressed
	 */
	public void draged(MouseEvent event, Ship ship) {
		if (event.isPrimaryButtonDown()) {
			if (ship.getX() + event.getX() <= 400 && ship.getX() + event.getX() > 0 && ship.getY() + event.getY() <= 400
					&& ship.getY() + event.getY() > 0) {
	...
	public void released(MouseEvent event, Ship ship) {
		...
		// if the position where it was released is invalid draw it in the last valid
		// position, if valid fill the previous occupied position with zeros and fill
		// the new occupied position with the ship id and draw it
		if (event.getButton() == MouseButton.PRIMARY) {
			...
			if (ship.isInvalid(posX, posY, ocPositions)) {
				...
				occupyPositions((int) ship.getLastValidX() / dimension, (int) ship.getLastValidY() / dimension,
						ship.getVertical(), ship.getTamanho(), ocPositions, 0);
				System.out.println();
	 ...
	 * @param tamanho           the height of the ship, used to determine how many
	 *                          cell it will fill
	 * @param occupiedPositions matrix of integers to signal unoccupied positions,
	 *                          position of the ships and position of the shots
	 ...
	 * @param ocPositions matrix of integers to signal unoccupied positions,
	 *                    position of the ships and position of the shots
	 */
	public void givePosition(Ship ship, int[][] ocPositions) {
		...
		occupyPositions(x / dimension, y / dimension, ship.getVertical(), ship.getTamanho(), ocPositions,
				ship.getTamanho());
	 ...
	 * @param ocPositions matrix of integers to signal unoccupied positions,
	 *                    position of the ships and position of the shots
	 * @param isEnemy     boolean to determine if the ship is a player's ship or a
	 *                    enemy's ship
	...
	public void playerTurn(int y, int x) {
		...
		if (occupiedPositionsEnemy[y][x] != 10) {
			...
			if (occupiedPositionsEnemy[y][x] != 0) {
				...
				// enemyBoard[x][y].setFill(hitShipPattern);

				Image hitShipImage = new Image(GameController.class.getResourceAsStream("Explosion(1).png"));
				ImageView hitShipView = new ImageView();
				enemyPane.getChildren().add(hitShipView);
				hitShipView.setImage(hitShipImage);
				hitShipView.setTranslateX(x * 40);
				hitShipView.setTranslateY(y * 40);
				...
				enemyBoard[x][y].setFill(hitOceanPattern);
	...
	public void computerTurn() {
		...
		if (occupiedPositions[y][x] != 0) {
			Image hitShipImage = new Image(GameController.class.getResourceAsStream("Explosion(1).png"));
			ImageView hitShipView = new ImageView();
			playerPane.getChildren().add(hitShipView);
			hitShipView.setImage(hitShipImage);
			hitShipView.setTranslateX(x * 40);
			hitShipView.setTranslateY(y * 40);
			...
			playerBoard[x][y].setFill(hitOceanPattern);
	...
	/**
	 * if all the ships is with zero life it set gameOver as true otherwise set as
	 * false
	 * 
	 * @param ships array of ship to iterate over and verify if every ship is with
	 *              zero life
	 */
